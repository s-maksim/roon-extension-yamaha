 ...
 */
// Modified to prevent other UPnP devices than Yamaha to be reported
...
class YamahaYXC {
	...
	async discoverSSDP(timeout) {
		let p = new Promise(async function(resolve, reject) {
			...
			ssdp.addService("urn:schemas-upnp-org:device:MediaRenderer:1");  // ADDED
			// Start
			ssdp.start();
	...
	async getXML(ssdpheader, timeout) {
			...
			const req = http.request(options, (res) => {
				...
				res.on('end', () => {
					try {
						...
						// console.log("Not a Yamaha");
						reject(error);
					}
					var isYamaha = reyxcControl.test(responseBody); // ADD 
					var model = reModelName.exec(responseBody);
					var name = reFriendlyName.exec(responseBody);
					// var sysid = reUniqueID.exec(responseBody); // COMMENT
					// var devid = reDevId.exec(responseBody); // COMMENT
					resolve([ name[1], model[1], isYamaha ]); // EDIT sysid[1], String(devid[2]).toUpperCase(), 
				});
			});
			// ADD timeout
			req.on('timeout', () => {
				...
				await Promise.all(
					foundDevices.map(async (device) => {
						...
						musicdevices.push({
							...
							model: deviceinfo[0],
							name: deviceinfo[1],
							isYamaha: deviceinfo[2],
							// systemId: deviceinfo[2], EDIT
							// deviceId: deviceinfo[3], EDIT
						});	
					})
				);
				console.log("musicdevices");
				console.table(musicdevices);
